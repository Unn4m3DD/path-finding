    ...
    for (let x = 0; x < width; x++) {
        for (let y = 0; y < height; y++) {
            if (maze[x][y].color_id === 1)
                table.children[y].children[x].style.backgroundColor = "#000";
            if (maze[x][y].color_id === 2)
                table.children[y].children[x].style.backgroundColor = "blue";
            if (maze[x][y].color_id === 3)
                table.children[y].children[x].style.backgroundColor = "green";
            if (maze[x][y].color_id === 4)
                table.children[y].children[x].style.backgroundColor = "#aaa";
            if (x === source.x && y === source.y)
                table.children[y].children[x].style.backgroundColor = "green";
            if (x === target.x && y === target.y)
                table.children[y].children[x].style.backgroundColor = "red";
        }
    }
};
...
const depth_first_search_rec = async (pos) => {
...
const a_star = async () => {
    ...
    while (queue.length != 0) {
        ...
        if (maze?.[temp.x]?.[temp.y - 1]?.color_id === 0 && !inArray(queue, { x: temp.x, y: temp.y - 1 }))
            queue.push({ x: temp.x, y: temp.y - 1, prev: { x: temp.x, y: temp.y } });
        queue.sort((p1, p2) => ((p1.x - target.x) * (p1.x - target.x) + (p1.y - target.y) * (p1.y - target.y) <
